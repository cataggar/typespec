// Minimal Node.js built-ins shims for WASI JS runtimes.
//
// Purpose:
// - `jco componentize` evaluates top-level JS in a WASI JS runtime which does not provide Node.js built-ins.
// - We shim the handful of built-ins that show up in the compiler dependency graph so the module graph can load.
//
// Non-goals:
// - This is NOT a full Node runtime. Most APIs throw if used.

export const nodeGlobalsBanner = `// Inject minimal Node globals for componentize-js evaluation (not a full Node runtime).
...
export function createNodeBuiltinsShimPlugin() {
  return {
